...
In addition to these principles, there are a couple of other points that the authors bring up that would be worth consideration:\newline
...
Applying the multipliers as discussed above yields:\newline 
$$0.6*1.42583*4430894=2022 Impact Lower Range$$\newline 
$$0.7*1.42583*4430894=2022 Impact Upper Range$$\newline 
And we can do the same for the 2023 data: \newline 
$$0.6*1.45283*3099362=2023 Lower Impact Range$$\newline 
$$0.7*1.45283*3099362=2023 Upper Impact Range$$\newline 
These calculations yield the following values: \newline 
* 2022 Range: (3790620,4422390)\newline 
* 2023 Range: (2651500,3093410)\newline 
* Combined:   (6442120,7515800)\newline 
...
Additional calculations made further in the project will be separate for each sport, and potentially for each age group depending on how great the difference is for spending between ages. These numbers will then be summed to find an overall impact estimate for all tournaments. Then, this impact estimate will be multiplied by the appropriate multipliers in order to gain further insight into the impact on the regional economy.\newpage 

## Part 4 Input/Output Models 

Here we hope to go into some detail about what an Input/Output model is, and what is used for in economic analysis.\newline 
 The Leontief matrix, also known as the Input-Output matrix or the Leontief input-output model, is a fundamental component of input-output analysis in economics. It was developed by the Russian-American economist Wassily Leontief in the 1930s as a way to model the interdependencies among various sectors of an economy. The Leontief matrix represents the flow of goods and services between sectors within an economy.\newline 

Here's a more detailed explanation of the Leontief matrix and its key components:\newline 

1. Matrix Structure:\newline 
   - The Leontief matrix is a square matrix, meaning it has the same number of rows and columns.\newline 
   - Each row and column in the matrix corresponds to a specific economic sector within the analyzed region or economy.\newline 

2. Elements of the Matrix:\newline 
   - The elements (entries) of the matrix represent the amount or value of goods and services purchased by each sector from all other sectors, including itself.\newline 
   - Typically, the entries are represented as aij, where "i" refers to the row (the purchasing sector) and "j" refers to the column (the selling sector).\newline 

3. Interdependence:\newline 
   - The Leontief matrix captures the interdependence of economic sectors. It shows how much each sector depends on other sectors for intermediate inputs.\newline 
   - For example, the construction sector might purchase steel from the manufacturing sector, and in turn, the manufacturing sector might purchase construction services for its buildings. This interdependence is reflected in the matrix.\newline 

4. Diagonal Elements:\newline 
   - The diagonal elements (aii) represent the "self-purchases" of each sector. This is the amount of goods and services a sector produces and consumes itself.\newline 
   - Self-purchases are usually found on the diagonal of the matrix because each sector buys from itself.\newline 

5. Off-Diagonal Elements:\newline 
   - The off-diagonal elements (aij, where $i\neq j)$ represent the purchases by one sector from another sector. These entries quantify the flow of goods and services between sectors.\newline

6. Use in Economic Analysis:\newline
   - The Leontief matrix is a critical tool for economic analysis, especially for input-output analysis and computable general equilibrium modeling.\newline
   - It allows economists to estimate the ripple effects of changes in one sector on the entire economy. For example, if there's an increase in consumer demand for automobiles, the Leontief matrix can help calculate the resulting increase in production and employment in the automotive sector, as well as its effects on other related sectors.\newline

7. Multipliers:\newline
   - The Leontief matrix is used to calculate economic multipliers, such as the output multiplier, income multiplier, and employment multiplier. These multipliers show the total economic impact of changes in demand or production in a specific sector.\newline

8. Data Source:\newline
   - The data used to populate the Leontief matrix comes from various sources, including national or regional economic surveys, input-output tables, and industry reports.\newline
...
Additionally, some notes about the multipliers the Input/Output model generates: \newline 
FINAL DEMAND MULTIPLIERS:\newline  In addition to the final demand output multiplier, there are three other types of multipliers: Earnings, Employment, and Value added. \newline 
...
1.	Purchases by consumers outside of the region. \newline 
2.	Investment in new buildings, equipment, and software\newline
3.	Purchases by government \newline 
4.	Purchases by households. \newline 

Additionally, there are Type I and Type II multipliers to consider as they are calculated to reflect different changes in the economy. For more detail about the multipliers generated by the Input/Output models: \newline 
Example of Multiplier: \newline
The sales multiplier for the lodging sector was 1.74 for the state of florida in 1996. That means that a visitor spending $100 on lodging will have total effects of 174 on sales within the state. That is, 100 received by the hotel as a direct effect, and 74 received by other related industries in the region through secondary effects. \newline 
...
Indirect and Induced effects: are the secondary effects resulting from the initial visitor spending. Indirect effects are sales income or jobs resulting from various rounds of purchases the hotel made to other “backward linked” industries in the region. \newline 
Induced effects are the sales income and jobs resulting from household spending of income earned as a result of visitor spending either directly or indirectly. \newline

*** Type 1 multipliers only capture the indirect effects, Type II multipliers include both induced and indirect effects. \newline 
