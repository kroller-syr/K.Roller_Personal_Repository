...
$\qquad$Lastly, we can look at the fees charged to the tournament directors by month over the three years. 
```{r,fig.align='center',echo=FALSE}
...
$\qquad$Another point of reference has been "The Economic Impact of 30 Sports Tournaments, Festivals, and Spectator Events in Seven US Cities" by Crompton and Lee. In the study, Crompton and Lee bring to light some important considerations for a study of this type.\newline 

$\qquad$ One of the main ones being, and rather important to our goals, is that the economic impact on a community when a tournament does not require an overnight stay is small. Typically one should not expect expenditures for these trips to exceed a range of 55 to 100 dollars (in 2000 USD). A follow up to this point is that a large event with a large amount of attendees is unlikely to have a large impact or any impact if 'time-switchers' are not accounted for.\newline 
...
$\qquad$Capture rate aside, this study has proven to be extremely useful as a guide, not only on what various methods are but the considerations to keep in mind when attempting to select a method or perform an analysis.\newpage
...
$\qquad$I-O models can measure the impact of an industry’s production on other industries in two ways. In a backward-linkage model, an increase in
...
$\qquad$I-O models assume that industries do not change the
...
$\qquad$I-O models assume that all businesses in an industry use
...
$\qquad$I-O models are often referred to as “fixed price” models
...
$\qquad$RIMS II is based on national I-O relationships that are
...
$\qquad$RIMS II is a single region I-O model. It ignores any feedback that may exist among regions.
...
### No time dimension.
 $\qquad$The length of time that it takes for the total impact of an initial change in economic activity to be completely realized is unclear because time is not explicitly included in I-O models. The actual adjustment period varies and is
...
$\qquad$Additionally, there are Type I and Type II multipliers to consider as they are calculated to reflect different changes in the economy. For more detail about the multipliers generated by the Input/Output models: \newline 
...
$\qquad$Induced effects are the sales income and jobs resulting from household spending of income earned as a result of visitor spending either directly or indirectly. \newline
